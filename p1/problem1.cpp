...
static void *lookupWorker(void *arg)
{
    ...
    for (size_t i = wargs->start; i < wargs->end; ++i)
    {
        wargs->result[i] = wargs->ht->get_value(wargs->key_list[i]).second;
    }
    pthread_exit(nullptr);
    return nullptr;
}
...
int main(int argc, char *argv[])
{
    ...
    for (uint32_t i = 0; i < runs; i++)
    {
        ...
        start = HR::now();
        batch_insert(ht, h_kvs_insert, add_result);
        end = HR::now();
        ...
        if (REM > 0)
        {
            bool del_result[DELETE];
            memset(del_result, 0, sizeof(bool) * DELETE);
            start = HR::now();
            batch_delete(ht, h_keys_del, del_result);
            end = HR::now();
        ...
        if (FIND > 0)
        {
            uint32_t find_result[FIND];
            memset(find_result, 0, sizeof(uint32_t) * FIND);
            start = HR::now();
            batch_search(ht, h_keys_lookup, find_result);
            end = HR::now();
